...
user: $(INIT_ELF) $(HOSTED_ELFS) $(NS_ELF) $(KBD_ELF) $(QTTY_ELF) $(FB_ELF) $(WM_ELF) $(INP_ELF) $(DISK_ELF) $(DISKTEST_ELF) $(VFS_ELF) $(NET_ELF) $(QSH_ELF) $(ECHO_ELF) $(LS_ELF) $(CAT_ELF) $(LOGIN_ELF) $(PS_ELF) $(IPCPING_ELF) $(PING_ELF) $(SHUTDOWN_ELF) $(CAPDEMO_ELF) $(THREADTEST_ELF) $(SOCKTEST_ELF) $(FSTEST_ELF) $(WMDEMO_ELF) $(WMTYPE_ELF) $(CWC_ELF) $(DTEST_ELF) $(DCHILD_ELF) $(MOUSETEST_ELF) $(RUNTESTS_ELF) $(NETTEST_ELF) $(ENVTEST_ELF)
...
USR_PROGRAMS  := disktest:DISKTEST qsh:QSH echo:ECHO ls:LS cat:CAT \
                 ...
                 shutdown:SHUTDOWN dtest:DTEST dchild:DCHILD capdemo:CAPDEMO threadtest:THREADTEST socktest:SOCKTEST fstest:FSTEST wm:WM wmdemo:WMDEMO wmtype:WMTYPE mousetest:MOUSETEST runtests:RUNTESTS nettest:NETTEST
