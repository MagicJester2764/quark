...
pub extern "C" fn _start() -> ! {
    ...
    // A numeric argument names a TID directly. No service name is numeric,
    // so this is unambiguous. Pinging it takes the right to call it, which
    // comes from looking its name up — so a task that registered no name, the
    // shell or another user program, is reported unreachable.
